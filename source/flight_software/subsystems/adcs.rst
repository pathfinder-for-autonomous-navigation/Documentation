...
Internally within ADCS Software, there is a Hardware Availablility Table that reports the functionality state
...
all ``Device`` ``setup()`` calls are succesful, the initial HAVT table will be represented by a bitset of all
``1``s. 
...
Therefore, nominally the reset and disable command registers are all commanded as ``0`` from the 
FSW Teensy. 
